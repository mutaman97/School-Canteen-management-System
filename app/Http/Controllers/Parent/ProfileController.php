...
class ProfileController extends Controller
{
    ...
    {
        $info=Student::find(Auth::guard('studentparent')->id());
        $storesCount = 5;
        $fund=500;
...
//        }
        return view('parent.my_profile',compact('info','storesCount','fund','totalEarnings'));
    }
    ...
    public function genUpdate(Request $request)
    {
        ...
        $user=Student::find(Auth::guard('studentparent')->id());
...
//        $user->subscribed_to_newsletter=$request->has('subscribed_to_newsletter');
    ...
    public function updatePassword(Request $request)
    {
        ...
        $info=Student::where('id', Auth::guard('studentparent')->id())->first();

        $check=Hash::check($request->oldpassword, Auth::guard('studentparent')->user()->password);

        if ($check==true) {
            Student::where('id', Auth::guard('studentparent')->id())->update(['password'=>Hash::make($request->password)]);
